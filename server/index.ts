...
app.post('/api/cards/create', async (req, res) => {
  ...
  const pan = Array(16).fill(0).map(() => Math.floor(Math.random() * 10)).join('');
...
app.get('/api/selectorProfiles', async (req, res) => {
  ...
  if (domain && userId) {
    const { data, error } = await query.eq('domain', domain).eq('user_id', userId).maybeSingle();
    if (error) return res.status(500).json({ error: error.message });
...
app.post('/api/selectorProfiles', async (req, res) => {
  ...
  console.log(`[API] Saving selector for ${domain} (${fieldType}): ${selector}`);
  ...
  try {
    ...
    if (fieldType === 'cardNumber') {
        updates.cardnumberselectors = [...new Set([...currentCardNumber, selector])];
    } else if (fieldType === 'cardExpiry') {
        updates.cardexpiryselectors = [...new Set([...currentCardExpiry, selector])];
